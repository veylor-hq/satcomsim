...
from src.ui.satellite_info_panel import SatelliteInfoPanel
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.m_sim_display = SimulationDisplay(
            GuiConstants.fps, self, "3D simulation rendering"
        )
    ...
    def hovered_satellite_slot(self, action):
        ...
        if (
            action.text() != "Add new satellite"
            and action.text() != "Configure"
            and action.text() != "Remove"
        ):
        ...
        if self.m_sim_display is not None:
            ...
            if new_window.exec_():
                ...
                self.m_sim_display.load_texture(
                    self.m_sim_display.sim().get_planet().get_img_path(), 0
                )
                ...
                self.m_sim_display.timer().setInterval(
                    int(
                        1000
                        * self.m_sim_display.sim().dt
                        / self.m_sim_display.sim().speed
                    )
                )
                self.update_sat_menu()

            # Update monitor size
            self.m_monitor.on_resize(
                self.m_sim_display.width(), self.m_sim_display.height()
            )
    ...
    def open_slot(self):
        ...
        if self.m_sim_display is not None:
            ...
            file_path, _ = QFileDialog.getOpenFileName(
                self, "Open simulation", os.getcwd(), "Simulations (*.sim)"
            )
            ...
            if file_path:
                try:
                    ...
                    # Allocate simulation
                    planet = Planet(
                        planet_mu, planet_radius, planet_day, planet_name, planet_img
                    )
                    ...
                    QMessageBox.critical(
                        self, "Error", f"Error opening simulation {file_path}: {str(e)}"
                    )
        ...
        if self.m_sim_display is not None and self.m_sim_display.sim() is not None:
            ...
            if file_path:
                try:
                    ...
                    result = self.m_sim_display.sim().save_to_file(
                    ...
                    QMessageBox.critical(
                        self, "Error", f"Error saving simulation: {str(e)}"
                    )
    ...
    def update_sat_menu(self):
        ...
        if self.m_sim_display and self.m_sim_display.sim():
            for i in range(self.m_sim_display.sim().nsat()):
                sat_menu = self.menu_sat.addMenu(
                    self.m_sim_display.sim().sat(i).get_name()
                )
