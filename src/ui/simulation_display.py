...
from src.ui.track_ball_camera import TrackBallCamera
...
class SimulationDisplay(SimulationGL):
    ...
    def __init__(
        self, frames_per_second=GuiConstants.fps, parent=None, name=None, sim=None
    ):
        """
        ...
        if self.m_sim is not None:
            ...
            self.planet_night_texture_path = (
                self.m_sim.get_planet().get_night_img_path()
            )
    ...
    def initializeGL(self):
        ...
        if self.planet_night_texture_path:
            self.load_texture(self.planet_night_texture_path, 1)
        self.load_texture("src/assets/images/gold_texture.jpg", 2)
        self.load_texture("src/assets/images/solar_panel_2.jpg", 3)
    ...
    def load_texture(self, path, i):
        ...
        glTexImage2D(
            ...
            GL_RGBA,
            GL_UNSIGNED_BYTE,
            qim_texture.bits().asstring(qim_texture.byteCount()),
        )
    ...
    def paintGL(self):
        ...
        if self.m_sim is not None:
            ...
            sun_angle = 360.0 * (
                math.fmod(self.m_sim.t, self.m_sim.get_planet().get_day())
    ...
    def _get_satellite_at_position(self, pos):
        ...
        mouse_y = (
            pos.y() * device_pixel_ratio
        )  # Don't flip y coordinate - we'll compare in screen space
        ...
        for i in range(self.m_sim.nsat()):
            ...
            if screen_pos:
                ...
                screen_y = (
                    viewport[3] - screen_pos[1]
                )  # Flip y coordinate to match mouse coordinates
                ...
                # Print debug info
                print(
                    f"Satellite {sat.get_name()}: screen pos ({screen_x/device_pixel_ratio:.1f}, {screen_y/device_pixel_ratio:.1f}), "
        ...
        if closest_sat:
            print(
                f"Selected satellite: {closest_sat.get_name()} at distance {min_distance/device_pixel_ratio:.1f} pixels"
            )
        else:
            print(
                f"No satellite selected. Click position: ({mouse_x/device_pixel_ratio:.1f}, {mouse_y/device_pixel_ratio:.1f})"
            )
