...
class SatelliteWindow(QDialog):
    ...
    def __init__(self, is_new, sat, planet, parent=None):
        ...
        self.om_small_box.setToolTip(
            f"[{self.om_small_box.minimum()}, {self.om_small_box.maximum()}]"
        )
        ...
        period = (
            1.0
            / Constants.twopi
            * math.sqrt(pow(self.a_box.value(), 3.0) / self.m_planet.get_mu())
        )
        ...
        if not is_new:
            ...
            self.a_box.setMinimum(
                self.m_planet.get_radius() / (1.0 - self.m_sat.get_orbit().get_e())
            )
            self.a_box.setToolTip(f"[{self.a_box.minimum()}, {self.a_box.maximum()}]")
            self.e_box.setValue(self.m_sat.get_orbit().get_e())
            self.e_box.setMaximum(
                1.0 - self.m_planet.get_radius() / self.m_sat.get_orbit().get_a()
            )
            self.e_box.setToolTip(f"[{self.e_box.minimum()}, {self.e_box.maximum()}]")
    ...
    def import_from_norad_slot(self):
        ...
        if not norad_id:
            QMessageBox.warning(
                self, "Invalid NORAD ID", "Please enter a valid NORAD ID."
            )
            return
